...
# Download the corpus in istex with different formats : tei, txt, pdf: need to have an access to the API if you need full text of articles but no need for the metadata
# Select the documents according to the publication date. We also make a selection to download only  documents containing the key phrase "Mental Rotation" in the meta-data
